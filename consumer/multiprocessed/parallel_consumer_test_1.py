...
def read_from_socket(server_address, server_port_for_file_names, server_port_for_file_contents, file_name_chunks, file_content_chunks, all_file_name_chunks, all_file_content_chunks):   
...
def consume():
    ...
    for _ in range(1):    
        ...
        read_from_socket(CONF['common']['SERVER_ADDRESS'], CONF['consumer_1']['SERVER_PORT_FOR_FILE_NAMES'], CONF['consumer_1']['SERVER_PORT_FOR_FILE_CONTENTS'], file_name_chunks, file_content_chunks, all_file_name_chunks, all_file_content_chunks)
        # process_before_writing(file_name_chunks, file_content_chunks, file_names, file_contents)
        
        file_names = all_file_name_chunks.split(b'.json')
        while all_file_content_chunks.count(b'}')>1 :
            first_file_ends_at = all_file_content_chunks.find(b'}')
            file_contents.append(all_file_content_chunks[:(first_file_ends_at+1)])
            all_file_content_chunks = all_file_content_chunks[(first_file_ends_at+1):]

        print(len(file_names))
        print(len(file_contents))
        
        # write_to_disk(file_names, file_contents, CONF['common']['BASE_PATH'])

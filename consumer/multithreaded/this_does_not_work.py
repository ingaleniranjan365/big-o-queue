...
def read_from_socket(server_address, server_port_for_file_names, server_port_for_file_contents, file_name_chunks, file_content_chunks, socket_reading_termination_signal):   
    ...
    socket_reading_termination_signal.set()       
...
def process_before_writing(file_name_chunks, file_content_chunks, file_names, file_contents, socket_reading_termination_signal):
    file_name_chunk=b''
    file_content_chunk=b''
    while not (socket_reading_termination_signal.is_set() and file_name_chunks.empty() and file_content_chunks.empty()):
...
def consume():
    thread_per_type_per_process = 6
    for _ in range(thread_per_type_per_process):
        # buffers
        ...
        socket_reading_termination_signal = threading.Event()  # shared signal

        threads = [
            threading.Thread(target=read_from_socket, kwargs=dict(
                server_address=SERVER_ADDRESS,
                server_port_for_file_names=SERVER_PORT_FOR_FILE_NAMES,
                server_port_for_file_contents=SERVER_PORT_FOR_FILE_CONTENTS,
                file_name_chunks=file_name_chunks,
                file_content_chunks=file_content_chunks,
                socket_reading_termination_signal=socket_reading_termination_signal
            )),
            threading.Thread(target= process_before_writing, kwargs=dict(
                file_name_chunks=file_name_chunks,
                file_content_chunks=file_content_chunks,
        ...
        # start writing to disk once reading and processing is done
...
if __name__ == "__main__":
    ...
    # consume()
    Parallel(n_jobs=2)(delayed(consume)() for i in range(2))
